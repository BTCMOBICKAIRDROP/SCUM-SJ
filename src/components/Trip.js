...
const Trip = (props) => {
  ...
  const dataMaxFromTrips = React.useMemo(() => {
    ...
    let maxT = 0;
    for (const t of trip) {
      if (t && t.timestamps && t.timestamps.length) {
  ...
  const dataMaxFromPassengers = React.useMemo(() => {
    ...
    for (const p of passengers) {
      if (p && p.request_time) {
        const requestTime = p.request_time;
        if (typeof requestTime === 'number' && requestTime > maxT) maxT = requestTime;
      }
    }
    return maxT;
  }, [passengers]);
  ...
  const visiblePassengers = React.useMemo(() => {
    ...
    return passengers.filter((p) => {
      const requestTime = p.request_time;
      return typeof requestTime === 'number' && requestTime <= time && (requestTime + 10) > time;
    });
  }, [passengers, time]);
  ...
  const layers = [
    ...
    new ScatterplotLayer({
      ...
      data: (() => {
        ...
        const arr = [];
        for (const t of trip) {
          if (!t || !t.path || !t.timestamps || t.path.length < 2 || t.timestamps.length < 2) continue;
          const drop = t.path[t.path.length - 1];
          const start = t.timestamps.length > 1 ? t.timestamps[1] : t.timestamps[0];
